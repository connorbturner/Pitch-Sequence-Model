### RNN Model for Conditional Pitch Sequencing; Embedded Sequence and Context
...
model_training_data = pd.read_csv('rnn_emb_model_data.csv')
...
def sequence_builder(sequence, num_pitches):
    ...
    new_sequences = np.full((n, 8, 4), -1)                              # Initialize sequence array
    context_values = np.zeros((n, 6 + (2 * num_pitches)))               # Initialize context vectors
    y_values = np.zeros((n, num_pitches))                               # Initialize target vectors

    idx = 8 + (2 * num_pitches)

    for i in range(n):
        new_sequences[i,0:i,:] = sequence[0:i, 0:4]                     # Record the sequence data
        context_values[i,:] = sequence[i, 2:idx]                        # Record the context data
...
for pitcher in pitcher_ids:
    ...
    pitches = sequences.iloc[:,-7:].values
    ...
    # Sequence embedding
    input_dims = (8, 15, 9, 13, 3, 9, 4, 10)                                                           # Input categories
    embedding_dim = 32                                                                                 # Embedding dimensions
    masked_sequence = layers.Masking(mask_value = -1)(pitch_sequence)                                  # Mask the padding in the sequence
    embedded_sequence = layers.Embedding(input_dims[0], embedding_dim)(masked_sequence[:, :, 0])       # Initialize sequence embedding
    for i in range(1, 4):                                                                              # For each feature:
        new_s_embedding = layers.Embedding(input_dims[i], embedding_dim)(masked_sequence[:, :, i])     # Build embedding layer
        embedded_sequence += new_s_embedding                                                           # Add to combined embedding layer

    # Context embedding
    embedded_context = layers.Embedding(input_dims[2], embedding_dim)(pitch_context[:, 0])             # Initialize context embedding
    for i in range(1,6):                                                                               # For each feature:
        new_c_embedding = layers.Embedding(input_dims[2+i], embedding_dim)(pitch_context[:, i])        # Build embedding layer
        embedded_context += new_c_embedding                                                            # Add to combined embedding layer
    new_context = layers.Concatenate()([pitch_context[:,-(2 * num_pitches):], embedded_context])       # Combine with pitch probabilities
    
    # RNN Layers
    lstm_1 = layers.LSTM(128, return_sequences = True, dropout = 0.1)(embedded_sequence)               # LSTM layers
    ...
    # Dense Layers
    dense_1 = layers.Dense(256, activation = 'relu')(combined_features)                                # Dense layers with ReLU activation
