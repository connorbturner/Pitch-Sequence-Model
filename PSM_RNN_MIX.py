### RNN Model for Conditional Pitch Sequencing; Embedded Sequence, One-Hot Context
...
model_training_data = pd.read_csv('rnn_mix_model_data.csv')
...
def sequence_builder(sequence, num_pitches):
    ...
    idx = 41 + (2 * num_pitches)

    for i in range(n):
        new_sequences[i,0:i,:] = sequence[0:i, 0:4]                     # Record the sequence data
        context_values[i,:] = sequence[i, 4:idx]                        # Record the context data
...
for pitcher in pitcher_ids:
    ...
    # Sequence embedding
    input_dims = (8, 15, 9, 13)                                                                        # Input categories
